...
def get_random_string_with_letters_digits(length: int) -> str:
    """
    Метод, возвращающий cлучайное строковое значение из букв английского алфавита (верхний и нижний регистр) и цифр
    :param length: длина строки
    :return строка заданной длины
    """
    result = random.choices(string.ascii_lowercase + string.ascii_uppercase + string.digits, k=length)
    return ''.join(result)
...
def get_random_float(before_dot: int = 1, after_dot: int = 2) -> float:
    """
    Метод генерирующий вещественное число
    ...
    if right_part.endswith('0'):
         right_part = right_part[:-1] + str(random.randint(1, 9)) + right_part[-1:-1]
